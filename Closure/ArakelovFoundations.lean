...
theorem C_S4_cert_gt_2sqrt13 : (C_S4_cert : ℝ) > 2 * Real.sqrt 13 := by linarith [sqrt_13_lt_4, C_S4_cert_gt_11]
...
theorem C_S4_cert_gt_2sqrt32 : (C_S4_cert : ℝ) > 2 * Real.sqrt 32 := by linarith [sqrt_32_lt_566, C_S4_cert_gt_1132]
...
theorem C_S5_cert_gt_2sqrt408 : (C_S5_cert : ℝ) > 2 * Real.sqrt 408 := by linarith [sqrt_408_lt_202, C_S5_cert_gt_4040]

/-! FIXED: generic wrapper — accepts Type, not just Prop. No more Prop vs Type mismatch -/
def Deligne1974_OPEN {α : Sort _} (x : α) : α := x
def SelbergWeilBC6_OPEN {α : Sort _} (x : α) : α := x
def BostConnesGRH_OPEN {α : Sort _} (x : α) : α := x
