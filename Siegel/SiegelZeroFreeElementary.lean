/-
  ...
  STEP A (PROVED): Algebraic identity for Re(s) > 1.
    ...
    Proof: lfunction_eq_eta_factor below.
    Route: ZMod.LFunction_eq_LSeries → shift index → altChar_succ_cast →
           tsum_even_add_odd → factor 2^s via mul_cpow_ofReal_nonneg →
           identify with ζ via zeta_eq_tsum_one_div_nat_add_one_cpow.
  ...
  PROOF STRUCTURE:
    ...
    lfunction_eq_eta_factor (PROVED): Step A — Re(s)>1 algebraic identity
    eta_identity           (1 SORRY): (1−2^{1−σ})·ζ(σ).re = η(σ)
                                      Step D (Abelian theorem) is the only remaining sorry
...
/-! ## § 4 (continued). The eta identity — 1 SORRY (hasSum_alternating_Dirichlet only) -/
...
lemma eta_identity (σ : ℝ) (hσ0 : 0 < σ) (hσ1 : σ < 1) :
  ...
  --    for all s with Re(s) > 1 — proved as lfunction_eq_eta_factor above.
  have hA : ∀ s : ℂ, 1 < s.re →
      ZMod.LFunction altChar s = (1 - (2 : ℂ) ^ (1 - s)) * riemannZeta s :=
    fun s hs => lfunction_eq_eta_factor s hs
